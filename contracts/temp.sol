...
contract Temp {
    struct Node {
        uint256 nodeId;
        string nodeData;
        address[] neighbourNode;
    }
    event Temp();

    uint256 public tempData = 100;

    constructor(address tempAddr, uint256 tempNum) {
        emit Temp();
    }

    function tempFunc1(address tempAddr, uint256 tempNum) public {
        tempData = tempNum;
        emit Temp();
    }
...
contract RequestReceiver {
    ...
    mapping(uint256 => Message) public tempMess;
